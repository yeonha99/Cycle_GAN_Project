...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data = {}
        if self.data_type == 'a' or self.data_type == 'both':           # dataset a를 호출하는 코드
            data_a = plt.imread(os.path.join(self.data_dir_a, self.lst_data_a[index])).squeeze()
        ...
        if self.data_type == 'b' or self.data_type == 'both':           # dataset b를 호출하는 코드

            data_b = plt.imread(os.path.join(self.data_dir_b, self.lst_data_b[index])).squeeze()
