...
def train(args):
    ...
    ## 네트워크 생성하기
    if network == "CycleGAN":                                                                           # 2개의 generator인 F,G 생성
        netG_a2b = CycleGAN(in_channels=nch, out_channels=nch, nker=nker, norm=norm, nblk=9).to(device) # G를 통해 x domain에서 y domain으로 변환 a,b의 domain에는 각 이미지만 넣기(실제,포켓몬)
        netG_b2a = CycleGAN(in_channels=nch, out_channels=nch, nker=nker, norm=norm, nblk=9).to(device) # F를 통해 y domain에서 x domain으로 변환

        netD_a = Discriminator(in_channels=nch, out_channels=1, nker=nker, norm=norm).to(device) # 2개 domain의 discriminator 생성
